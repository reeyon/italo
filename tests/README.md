...
Core tests take longer than any other Italo tests, due to the high amount of computational work involved in validating core components.
...
To run only Italo's core tests (after building):
...
To run only Italo's crypto tests (after building):
...
```
italod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `italo-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Italo's hash tests (after building):
...
To run only Italo's performance tests (after building):
...
To run only Italo's unit tests (after building):
